...
A robust, enterprise-grade multi-tenant application designed for seamless project and task orchestration. Powered by modern technologies including Node.js, Express, React, and PostgreSQL. Complete with Docker containerization for effortless deployment.

## ✨ Core Capabilities

- **Isolated Multi-Tenancy** - Complete data segregation across organizational boundaries
- **Granular Permission System** - Three-tier access model: super_admin, tenant_admin, and standard user
- **Secure Token-Based Auth** - JWT tokens with 24-hour lifecycle and bcrypt password encryption
- **Comprehensive REST API** - 19 fully-featured endpoints covering all CRUD operations
- **Plan-Based Resource Limits** - Flexible subscription tiers with user and project quotas
- **Complete Audit Trail** - Automated logging of all critical system activities
- **Modern React Interface** - Intuitive UI with route protection and state management
- **Zero-Configuration Database** - Automated schema migrations and data seeding
- **Container-Ready Architecture** - Full Docker Compose orchestration with health monitoring
...
║              Port: 5432                                 ║
║  → Multi-tenant data models (Tenant, User, etc.)       ║
...
- Node.js 18.x or higher (local development only)
...
Expect all containers to display `Healthy` or `Running` state.
...
### 1. Access the Platform (Demo Mode)
Browse to **http://localhost:3000** and authenticate using:
...
Select "Register" to establish a new tenant organization with administrator credentials.

### 3. User Administration
Navigate to the Users section to:
- Browse all organizational users
